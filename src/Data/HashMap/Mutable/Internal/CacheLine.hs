...
module Data.HashMap.Mutable.Internal.CacheLine
  ( cacheLineSearch
  , cacheLineSearch2
  , cacheLineSearch3
  , forwardSearch2
...
prefetchRead :: PrimMonad m => IntArray (PrimState m) -> Int -> m ()
prefetchWrite :: PrimMonad m => IntArray (PrimState m) -> Int -> m ()
...
prefetchRead a i = unsafePrimToPrim $ prefetchCacheLine_read v x
  where
    v   = M.toPtr a
    x   = fI i

prefetchWrite a i = unsafePrimToPrim $ prefetchCacheLine_write v x
  where
    v   = M.toPtr a
    x   = fI i
...
forwardSearch2 :: PrimMonad m => IntArray (PrimState m) -> Int -> Int -> Elem -> Elem -> m Int
forwardSearch2 !vec !start !end !x1 !x2 =
    liftM fromEnum $! unsafePrimToPrim c
  where
    c = c_forwardSearch_2 (M.toPtr vec) (fI start) (fI end) (fI x1) (fI x2)


{-# INLINE forwardSearch3 #-}
forwardSearch3 :: PrimMonad m => IntArray (PrimState m) -> Int -> Int -> Elem -> Elem -> Elem -> m Int
...
{-# INLINE lineSearch #-}
lineSearch :: PrimMonad m => IntArray (PrimState m) -> Int -> Elem -> m Int
...
{-# INLINE lineSearch2 #-}
lineSearch2 :: PrimMonad m => IntArray (PrimState m) -> Int -> Elem -> Elem -> m Int
...
{-# INLINE lineSearch3 #-}
lineSearch3 :: PrimMonad m => IntArray (PrimState m) -> Int -> Elem -> Elem -> Elem -> m Int
...
lineSearch :: IntArray (PrimState m)        -- ^ vector to search
           -> Int               -- ^ start index
           -> Elem              -- ^ value to search for
           -> m Int          -- ^ dest index where it can be found, or
                                -- \"-1\" if not found
...
-- Required: unlike in C search, required that the start index is
-- cache-line-aligned and array has at least 32 elements after the start index
lineSearch2 :: IntArray (PrimState m)        -- ^ vector to search
            -> Int               -- ^ start index
            -> Elem              -- ^ value to search for
...
-- Required: unlike in C search, required that the start index is
-- cache-line-aligned and array has at least 32 elements after the start index
lineSearch3 :: IntArray (PrimState m)        -- ^ vector to search
            -> Int               -- ^ start index
            -> Elem              -- ^ value to search for
            -> Elem              -- ^ second value to search for
...
-- vector.
naiveSearch :: IntArray (PrimState m)       -- ^ vector to search
            -> Int              -- ^ start index
...
------------------------------------------------------------------------------
naiveSearch2 :: IntArray (PrimState m)       -- ^ vector to search
             -> Int              -- ^ start index
             -> Int              -- ^ number of things to search
             -> Elem             -- ^ value to search for
...
------------------------------------------------------------------------------
naiveSearch3 :: IntArray (PrimState m)       -- ^ vector to search
             -> Int              -- ^ start index
             -> Int              -- ^ number of things to search
             -> Elem             -- ^ value to search for
             -> Elem             -- ^ value 2 to search for
...
--
cacheLineSearch :: PrimMonad m
                => IntArray (PrimState m)        -- ^ vector to search
                -> Int               -- ^ start index
                -> Elem              -- ^ value to search for
                -> m Int          -- ^ dest index where it can be found, or
                                     -- \"-1\" if not found
